...
body::before{
  ...
  background:
    radial-gradient(ellipse 80% 60% at 10% 10%, rgba(79,142,255,.07) 0%, transparent 60%),
    radial-gradient(ellipse 60% 50% at 90% 80%, rgba(169,127,245,.055) 0%, transparent 55%),
    radial-gradient(ellipse 50% 40% at 50% 100%, rgba(0,212,240,.045) 0%, transparent 50%);
  animation:auroraShift 18s ease-in-out infinite alternate;
}
@keyframes auroraShift{
  0%{opacity:1;transform:scale(1) translate(0,0)}
  50%{opacity:.8;transform:scale(1.04) translate(-10px,12px)}
  100%{opacity:1;transform:scale(1) translate(8px,-8px)}
}
...
.glass{
  ...
  position:relative;
  overflow:hidden;
  transition:border-color .3s cubic-bezier(.16,1,.3,1),transform .35s cubic-bezier(.16,1,.3,1),box-shadow .35s cubic-bezier(.16,1,.3,1);
}
.glass::before{
  content:'';position:absolute;inset:0;
  background:radial-gradient(ellipse at 15% 0%,rgba(79,142,255,.08) 0%,transparent 65%);
...
.glass:hover::after{left:160%}
.glass:hover{border-color:rgba(255,255,255,.16)}

/* ── Stat card hover lift ── */
.stat-lift{cursor:default;transition:transform .35s cubic-bezier(.16,1,.3,1),box-shadow .35s cubic-bezier(.16,1,.3,1),border-color .3s}
.stat-lift:hover{
  transform:translateY(-7px) scale(1.01);
  box-shadow:0 32px 80px rgba(0,0,0,.75), 0 0 0 1px rgba(79,142,255,.12), 0 0 40px rgba(79,142,255,.06);
}

/* ── Section label ── */
.sh{font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:700;letter-spacing:1.6px;text-transform:uppercase;color:${T.sub};display:flex;align-items:center;gap:8px}

/* ── Table ── */
.tbl{width:100%;border-collapse:collapse;font-size:13px}
.tbl th{text-align:left;padding:12px 16px;font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:600;letter-spacing:1.2px;text-transform:uppercase;color:${T.sub};border-bottom:1px solid ${T.border}}
.tbl td{padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.04);vertical-align:middle;color:${T.text};transition:background .18s}
.tbl tbody tr{transition:background .15s}
.tbl tbody tr:hover td{background:rgba(79,142,255,.07)}
.tbl tbody tr:last-child td{border-bottom:none}

/* ── Badges ── */
.badge{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:700;letter-spacing:.7px;white-space:nowrap;transition:filter .2s,transform .15s}
.badge:hover{filter:brightness(1.15);transform:scale(1.04)}
.bw{background:rgba(0,224,154,.12);color:${T.win};border:1px solid rgba(0,224,154,.25);box-shadow:0 0 12px rgba(0,224,154,.08)}
.bl{background:rgba(255,64,96,.12);color:${T.loss};border:1px solid rgba(255,64,96,.25);box-shadow:0 0 12px rgba(255,64,96,.08)}
.bp{background:rgba(255,186,53,.09);color:${T.pend};border:1px solid rgba(255,186,53,.22);box-shadow:0 0 12px rgba(255,186,53,.07)}
.b1{background:rgba(79,142,255,.12);color:${T.accent};border:1px solid rgba(79,142,255,.28);box-shadow:0 0 12px rgba(79,142,255,.08)}
.bo{background:rgba(0,212,240,.09);color:${T.cyan};border:1px solid rgba(0,212,240,.24);box-shadow:0 0 12px rgba(0,212,240,.07)}
.ba{background:rgba(169,127,245,.11);color:${T.purple};border:1px solid rgba(169,127,245,.26);box-shadow:0 0 12px rgba(169,127,245,.07)}
.bs{background:rgba(58,74,99,.2);color:${T.sub};border:1px solid rgba(58,74,99,.4)}

/* ── Tabs ── */
.tab{
  position:relative;padding:9px 20px;border-radius:11px;cursor:pointer;
...
.tab-on{
  background:rgba(79,142,255,.16);
  border-color:rgba(79,142,255,.35);
  color:${T.accent};
  box-shadow:0 0 20px rgba(79,142,255,.12), inset 0 1px 0 rgba(255,255,255,.06);
}
.tab-on::after{
  content:'';position:absolute;bottom:-1px;left:50%;transform:translateX(-50%);
  width:50%;height:2px;border-radius:999px;
...
.sel{
  ...
  padding:8px 14px;border-radius:10px;font-family:'JetBrains Mono',monospace;font-size:11px;
  outline:none;cursor:pointer;transition:border-color .2s,box-shadow .2s;
  appearance:none;-webkit-appearance:none;
}
.sel:focus{border-color:rgba(79,142,255,.5);box-shadow:0 0 0 3px rgba(79,142,255,.1)}
.sel:hover{border-color:rgba(255,255,255,.14)}

/* ── Dot (live indicator) ── */
.dot{width:8px;height:8px;border-radius:50%;background:${T.win};flex-shrink:0}
.dot-live{animation:dpulse 2.4s ease-in-out infinite}
.dot-off{background:${T.loss}}
@keyframes dpulse{0%,100%{box-shadow:0 0 0 0 rgba(0,224,154,.65)}60%{box-shadow:0 0 0 8px rgba(0,224,154,0)}}
/* Ring effect */
.dot-live-wrap{position:relative;display:flex;align-items:center;justify-content:center;width:16px;height:16px}
.dot-live-ring{
  position:absolute;width:16px;height:16px;border-radius:50%;
  border:1px solid rgba(0,224,154,.4);
  animation:liveRing 2.4s ease-out infinite;
}
@keyframes liveRing{0%{transform:scale(.6);opacity:.8}100%{transform:scale(1.9);opacity:0}}

/* ── Log feed entry ── */
.log{border-radius:13px;padding:10px 14px;border:1px solid transparent;animation:lslide .3s cubic-bezier(.16,1,.3,1)}
...
.log-pred:hover{background:rgba(79,142,255,.1);border-color:rgba(79,142,255,.28)}
.log-res{background:rgba(0,224,154,.07);border-color:rgba(0,224,154,.18)}
.log-res:hover{background:rgba(0,224,154,.1)}
...
.log{transition:background .2s,border-color .2s;cursor:default}

/* ── Parlay pick row ── */
.ppick{font-family:'JetBrains Mono',monospace;font-size:11px;padding:9px 13px;border-bottom:1px solid rgba(255,255,255,.04);display:flex;align-items:flex-start;gap:8px;color:${T.text};line-height:1.5;transition:background .15s}
.ppick:hover{background:rgba(255,255,255,.025)}
.ppick:last-child{border-bottom:none}

/* ── Big stat number ── */
.bignum{
  font-family:'JetBrains Mono',monospace;font-size:28px;font-weight:700;
  letter-spacing:-1.5px;line-height:1;color:${T.text};
...
.ticker-wrap{
  overflow:hidden;
  background:linear-gradient(90deg,${T.s0} 0%,rgba(15,18,32,.6) 50%,${T.s0} 100%);
...
.ticker-wrap::before{left:0;background:linear-gradient(90deg,${T.bg},transparent)}
.ticker-wrap::after{right:0;background:linear-gradient(270deg,${T.bg},transparent)}
.ticker-inner{display:flex;gap:48px;animation:ticker 36s linear infinite;white-space:nowrap;padding:0 24px}
...
@media(max-width:768px){
  .bottom-nav{
    ...
    background:rgba(11,13,24,.92);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);
    border-top:1px solid ${T.border};
    z-index:100;padding:6px 0 calc(6px + env(safe-area-inset-bottom));
  }
  .bnav-item{flex:1;display:flex;flex-direction:column;align-items:center;gap:3px;cursor:pointer;padding:4px 0;transition:all .2s cubic-bezier(.16,1,.3,1);border:none;background:none;outline:none}
  .bnav-item span:first-child{font-size:20px;line-height:1;transition:transform .2s cubic-bezier(.16,1,.3,1)}
  .bnav-item span:last-child{font-size:10px;font-family:'JetBrains Mono',monospace;letter-spacing:.5px;transition:color .2s}
  .bnav-active span:first-child{transform:translateY(-1px)}
  .bnav-active span:last-child{color:${T.accent}}
  .bnav-off span:last-child{color:${T.muted}}
  .bnav-item:active span:first-child{transform:scale(.88)}
  /* Active indicator pill */
  .bnav-active::before{
    content:'';position:absolute;top:0;width:36px;height:2px;border-radius:999px;
    background:linear-gradient(90deg,transparent,${T.accent},transparent);
    animation:pillIn .3s cubic-bezier(.16,1,.3,1) forwards;
  }
  @keyframes pillIn{from{opacity:0;width:0}to{opacity:1;width:36px}}
}
...
/* ── Glow accents ── */
.glow-blue{box-shadow:0 0 24px rgba(79,142,255,.28)}
.glow-green{box-shadow:0 0 24px rgba(0,224,154,.28)}
...
/* ── Slide-in from left ── */
.slide-left{opacity:0;transform:translateX(-20px);transition:opacity .5s cubic-bezier(.16,1,.3,1),transform .5s cubic-bezier(.16,1,.3,1)}
...
@keyframes popFade{from{opacity:1;transform:translateY(0) scale(1)}to{opacity:0;transform:translateY(-10px) scale(.96)}}
.popup-in{animation:popSlide .42s cubic-bezier(.16,1,.3,1) forwards}
...
/* ── Fade up stagger for lists ── */
@keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
.fade-item{animation:fadeUp .4s cubic-bezier(.16,1,.3,1) both}
`;
...
const StatCard = ({ value, label, color, icon, sub }) => {
  ...
  return (
    <div className="glass stat-lift reveal" style={{ padding: "20px 22px" }}>
      {/* Colored glow bar top */}
      <div style={{ position: "absolute", top: 0, left: "20%", right: "20%", height: 2, borderRadius: "0 0 4px 4px", background: `linear-gradient(90deg, transparent, ${color}88, transparent)` }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ minWidth: 0 }}>
          <div className="bignum" style={{ color: color || T.text }}>{displayed}</div>
          <div style={{ fontSize: 11, color: T.sub, marginTop: 8, fontWeight: 500, letterSpacing: ".6px" }}>{label}</div>
          {sub && <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: T.muted, marginTop: 3 }}>{sub}</div>}
        </div>
        <div style={{
          width: 40, height: 40, borderRadius: 12, flexShrink: 0,
          background: `radial-gradient(ellipse at 30% 30%, ${color}28 0%, ${color}0e 100%)`,
          border: `1px solid ${color}30`,
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: 18,
          boxShadow: `0 0 20px ${color}18, inset 0 1px 0 rgba(255,255,255,.07)`,
...
const EmptyState = ({ icon, title, desc }) => (
  <div style={{ textAlign: "center", padding: "52px 24px" }}>
    <div style={{
      ...
      filter: "drop-shadow(0 0 12px rgba(79,142,255,.3))",
    }}>{icon}</div>
    <div style={{ fontSize: 14, fontWeight: 600, color: T.sub, marginBottom: 8 }}>{title}</div>
    <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 11, color: T.muted, lineHeight: 1.7, maxWidth: 300, margin: "0 auto" }}>{desc}</div>
  </div>
);
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (e) => {
        ...
        if (msg.event === "reset") {
          // Hard reset: kosongkan semua state lokal, reload dari backend
          setPreds([]);
          predsRef.current = [];
          setHistory([]);
          setHistoryTotal(0);
  ...
  useEffect(() => {
    ...
    const els = document.querySelectorAll(".reveal, .slide-left");
    els.forEach(el => observer.observe(el));
    return () => observer.disconnect();
  }, [tab, preds, history, stats]);
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 28 }}>
        ...
        <div style={{
          ...
          background: "linear-gradient(135deg,#1e3a7e 0%,#0b1840 100%)",
          border: "1px solid rgba(79,142,255,.4)",
          display: "flex", alignItems: "center", justifyContent: "center", fontSize: 32,
          boxShadow: "0 0 40px rgba(79,142,255,.3)",
          animation: "dpulse 2s ease-in-out infinite",
        }}>⚽</div>
        <div>
          <div style={{ fontFamily:"'JetBrains Mono',monospace", color: T.accent, fontSize: 12, letterSpacing: "4px", textAlign: "center", marginBottom: 18 }}>BOOTING SYSTEM</div>
          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            {[0,1,2,3].map(i => (
              <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: T.accent, animation: `dpulse 1.4s ${i*.18}s ease-in-out infinite`, opacity: .8 }} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <header style={{
        ...
        background: `rgba(7,8,15,.88)`, backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)",
        borderBottom: `1px solid rgba(255,255,255,.07)`,
        padding: "10px 20px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
        boxShadow: "0 1px 0 rgba(255,255,255,.04), 0 8px 32px rgba(0,0,0,.4)",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
          <div style={{
            ...
            boxShadow: "0 0 28px rgba(79,142,255,.25), inset 0 1px 0 rgba(255,255,255,.1)",
            flexShrink: 0,
          }}>⚽</div>
          <div>
            <h1 style={{ fontSize: 19, fontWeight: 700, letterSpacing: "-.5px", lineHeight: 1 }}>
              Parlay{" "}
              <span style={{
                background: "linear-gradient(135deg,#4F8EFF 0%,#00D4F0 100%)",
                WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
                backgroundClip: "text",
              }}>AI</span>
            </h1>
            <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 9, color: T.muted, letterSpacing: "2.5px", marginTop: 3 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {/* Win rate pill */}
          <div style={{
            background: `linear-gradient(135deg, ${T.win}1a 0%, ${T.win}0a 100%)`,
            border: `1px solid ${T.win}30`,
            borderRadius: 13, padding: "6px 16px",
            display: "flex", flexDirection: "column", alignItems: "flex-end",
            boxShadow: `0 0 20px ${T.win}10`,
          }}>
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 22, fontWeight: 700, color: T.win, letterSpacing: "-1px", lineHeight: 1 }}>
              {fmt(o?.win_rate ?? 0)}%
            </span>
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 8, color: T.muted, letterSpacing: "1.8px", marginTop: 1 }}>WIN RATE</span>
          </div>
          {/* WS status */}
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <div style={{ position: "relative", width: 16, height: 16, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {wsOk && <div className="dot-live-ring" />}
              <div className={`dot ${wsOk ? "dot-live" : "dot-off"}`} />
            </div>
            <span className="desktop-only" style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 10, color: wsOk ? T.win : T.loss, letterSpacing: "1.8px", fontWeight: 700 }}>
              {wsOk ? "LIVE" : "OFFLINE"}
            </span>
          </div>
        </div>
      ...
      {/* ── DESKTOP NAV TABS ── */}
      <div className="desktop-only" style={{ maxWidth: 1400, margin: "0 auto", padding: "16px 20px 0", display: "flex", gap: 5, alignItems: "center" }}>
        <div style={{
        ...
        {tab === "overview" && (
          <>
            ...
            <div className="chart-grid" style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 14, marginBottom: 16 }}>
              {/* Win rate chart */}
              <div className="glass reveal reveal-d1" style={{ padding: "22px 24px" }}>
                <div className="sh" style={{ marginBottom: 16 }}>
                  <span style={{ width: 7, height: 7, borderRadius: "50%", background: T.accent, display: "inline-block", boxShadow: `0 0 10px ${T.accent}` }} />
                  Running Win Rate
                </div>
                {chartWR.length === 0
              ...
              <div className="glass reveal reveal-d2" style={{ padding: "22px 24px" }}>
                <div className="sh" style={{ marginBottom: 16 }}>
      ...
      {/* ── FOOTER ── */}
      <div className="desktop-only" style={{ textAlign: "center", padding: "28px 0 36px" }}>
        <div style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 9, color: T.dim, letterSpacing: "2.5px" }}>
        ...
        <div style={{ marginTop: 10, display: "flex", alignItems: "center", justifyContent: "center", gap: 20 }}>
          {["PL","PD","SA","UCL","BL1"].map(c => (
            <span key={c} style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 8, color: T.muted, letterSpacing: "1px", opacity: .5 }}>{c}</span>
          ))}
          <span style={{ color: T.dim, fontSize: 8 }}>· · ·</span>
          {["FL1","UEL","MLS","BSA"].map(c => (
            <span key={c} style={{ fontFamily:"'JetBrains Mono',monospace", fontSize: 8, color: T.muted, letterSpacing: "1px", opacity: .5 }}>{c}</span>
          ))}
        </div>
      </div>
